...
function (Okta, Q, Logger, OktaAuth, Util, Expect, Router,
  ...
  Expect.describe('LoginRouter', function () {
    ...
    itp('triggers an render event when routing to default route and when status is UNAUTHENTICATED', function () {
      return setup({ stateToken: 'aStateToken' })
      .then(function (test) {
        Util.mockRouterNavigate(test.router);
        test.setNextResponse(resUnauthenticated);
    ...
    Expect.describe('Events', function () {
      ...
      itp('triggers a render event when first controller is loaded', function () {
        return setup()
        .then(function (test) {
          test.router.primaryAuth();
          return Expect.waitForPrimaryAuth(test);
      ...
      itp('triggers both pageRendered and render events when first controller is loaded', function () {
        return setup()
        .then(function (test) {
          test.router.primaryAuth();
          return Expect.waitForPrimaryAuth(test);
      ...
      itp('triggers a render event when navigating to a new controller', function () {
        return setup()
        .then(function (test) {
          // Test navigation from primary Auth to Forgot password page
          test.router.primaryAuth();
